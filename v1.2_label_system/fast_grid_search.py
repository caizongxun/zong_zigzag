...
class SmartGridSearch:
    ...
    def _get_search_space(self) -> Dict:
        return {
            'fib_proximity': (0.001, 0.01),
            'bb_proximity': (0.001, 0.01),
            'zigzag_threshold': (0.2, 1.0),
        }
    ...
    def _print_progress(self, completed: int, total: int, elapsed: float,
                        current_score: float, method: str = 'ADAPT'):
    ...
    def _adaptive_search(self, num_iterations: int = 30):
        ...
        print("適應性搜索 (最快方案)")
        print("="*70)
        print(f"總計評估: {num_iterations} 次")
        print(f"預計耗時: 10-15 分鐘")
        print(f"优势: 適應性探索 (探索 + 開強)")
        print("="*70)
        print()
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                self._print_progress(completed, num_iterations, elapsed, current_score, 'PHASE1')
            ...
            if all_candidates:
                ...
                for future in as_completed(futures):
                    ...
                    if completed <= num_iterations:
                        self._print_progress(completed, num_iterations, elapsed, current_score, 'PHASE2')

        print("\n")
        elapsed = time.time() - self.start_time
        print("="*70)
        print(f"搜索完成! 耗時: {elapsed:.1f} 秒 ({elapsed/60:.1f} 分鐘)")
        print(f"測試組合: {completed} 次")
        print(f"最優分数: {self.best_score:.2f}")
        print("="*70)
    ...
    def _genetic_algorithm(self, num_generations: int = 20, population_size: int = 30):
        ...
        print("遣傳演算法搜索")
        print("="*70)
        print(f"总計計代: {num_generations}")
        print(f"总計人口: {population_size}")
        print(f"总計計計: {num_generations * population_size}")
        print(f"預計耗時: 20-30 分鐘")
        print("="*70)
        print()
...
def main():
    print("\n" + "="*70)
    print("智能網格搜索 - 最优方案自动选择")
    print("="*70)
    
    # 自动选择適应性搜索 (最快)
    print("\n自动選選: 適應性搜索 (最快 10-15 分鐘)\n")
    ...
    print("搜索完成!")
    print("="*70)
    print("推薦配置: ./output/recommended_config.yaml")
    print("完整結果: ./output/grid_search_results.csv")
    print("="*70)
